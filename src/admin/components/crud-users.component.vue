...
import MembershipsCounterComponent from "./memberships-counter.component.vue";
...
export default {
  ...
  methods: {
    addCategoryOng(){
      ...
      axios.post('https://662adbcede35f91de15695c4.mockapi.io/cambiazo/users', {
        id: this.id,
        name: this.name,
        email: this.email,
        city: this.city,
    ...
    deleteCategoryOng(id){
      axios.delete('https://662adbcede35f91de15695c4.mockapi.io/cambiazo/users/'+id)
          .then((response)=> {
    ...
    editCategoryOng(id){
      ...
      this.membership = organization.membership;
      this.img = organization.img;
      this.idEdit=id;
    },
    confirmEdit(){
      const organization = this.Organizations.find((organization)=> organization.id === this.idEdit);
      axios.put('https://662adbcede35f91de15695c4.mockapi.io/cambiazo/users/'+organization.id, {
        id: organization.id,
        name: this.name,
        email: this.email,
        city: this.city,
          ...
          .then((response)=> {
            this.Organizations = this.Organizations.map((organization)=> {
              if(organization.id === this.idEdit){
                ...
                organization.membership = this.membership;
                organization.img = this.img;
              }
              return organization;
            });
          })
...
<template>
  <div style="background-color: #F8F7F4;">
    ...
    <h1 style="text-align: center; font-size: 30px; font-weight: bolder; padding-top: 2rem;">Usuarios</h1>
    <div style="text-align: center; font-size: 18px; justify-content: center; margin-bottom: 20px; display: flex;" >
      <memberships-counter-component></memberships-counter-component>
    </div>
    <div class="crud-container">
      <div class="input-crud">
        ...
        <pv-input v-model="email" id="email" required="true" class="input"></pv-input>

        <label for="title">City: </label>
        <pv-input v-model="city" id="city" required="true" class="input"></pv-input>

        <label for="title">Membership: </label>
        <pv-input v-model="membership" id="membership" required="true" class="input"></pv-input>

        <label for="title">Imagen: </label>
        <pv-input v-model="img" id="img" required="true" class="input"></pv-input>
      ...
      <div class="table-crud">
        <table>
          ...
          <tr v-for="organization in Organizations" :key="organization.id">
            ...
            <td><img :src="organization.img" alt="Logo Ong" width="80"></td>
            <td>
